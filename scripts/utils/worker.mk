...
clear-queue:
	$(COMPOSE) run --rm $(RUN_USER) $(SCRIPTS_MOUNT) worker npx ts-node src/scripts/queue.ts --clear
...
dump-queue:
	...
	$(COMPOSE) run --rm $(RUN_USER) $(SCRIPTS_MOUNT) -v $$(pwd)/data:/app/data worker npx ts-node src/scripts/queue.ts --dump
	@echo "🎉 Done! You can view the dump at: data/queue_dump.json"

fix-urls:
	@echo "=== 📊 [BEFORE] Redis Queue Status ==="
	$(COMPOSE) run --rm $(RUN_USER) $(SCRIPTS_MOUNT) worker npx ts-node src/scripts/queue.ts --status
