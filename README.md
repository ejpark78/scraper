...
특히, 구버전 셸 스크립트(`scripts/` 폴더 내 GNU 도구 종속성)를 완전히 제거하고 파일 탐색 및 문자열 파싱 로직을 Node.js 네이티브 정규식 엔진으로 이식하였으며, **`[jobId].html` 및 `[jobId].md` 표준 숫자 파일명 체계**를 도입하여 OS 파일 경로 길이 한계를 완벽히 해결했습니다. 또한 **인메모리 O(1) 캐시 맵 조회** 방식의 도입으로 수천 건 아카이빙 시에도 로컬 디스크 I/O 병목이 발생하지 않는 초고속 아키텍처를 실현했습니다.
...
### 4. `[jobId]` 표준 숫자 파일명 포맷
- **OS 파일 경로 길이 제약 극복**: 링크드인 공고 제목 및 회사명이 다국어로 비대하게 길어질 때 생기던 OS 파일 저장 불가 오류(Path Toolong Exception)를 방지하기 위해, 저장 형식을 숫자로 구성된 고유 번호인 `[jobId].html` 및 `[jobId].md` 구조로 표준화했습니다.
- **일관된 파일 네이밍**: 수집된 HTML 백업과 마크다운 요약이 모두 `[jobId]` 파일명으로 매칭되므로 데이터 관리 및 검색이 매우 간결해집니다.
...
```text
  ...
  📂 markdown/[Location]/[Date]/{JOB_ID}.md                                               📂 recent/markdown/{JOB_ID}.md
```
...
- **`[jobId]` 표준 숫자 파일명 체계 도입**: 지나치게 길거나 특수문자가 포함된 다국어 파일명이 야기하는 OS 경로 오류를 원천 차단하기 위해, 모든 백업 HTML 및 요약 MD의 파일명을 순수 `[jobId]` 구조로 개편 및 일원화했습니다.
