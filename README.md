...
### 1. 사전 요구 사항
시스템 환경에 **Node.js (18버전 이상)** 및 **Bash 환경**이 준비되어야 합니다.
...
### 1단계. 링크드인 목록 파싱 및 정밀 청소 (`get_urls.sh`)
추출하고자 하는 링크드인 공고 목록 웹페이지 소스를 `list/list.html` 경로로 저장한 뒤 스크립트를 기동합니다:
```bash
bash get_urls.sh
```
- 상대 주소와 파라미터가 모두 걸러진 고유 Canonical URL들이 정제되어 `list/urls.txt`에 누적 및 자동 병합됩니다.

### 2단계. 일괄 파이프라인 가동 및 분류 배정 (`get_posts.sh`)
가공된 `list/urls.txt`를 기반으로 다운로드 및 변환 파이프라인을 실행합니다:
```bash
bash get_posts.sh list/urls.txt
```
...
7. HTML 백업 파일과 생성된 마크다운 결과물을 각각 `html/[근무지]/[포스팅날짜]/` 및 `posts/[근무지]/[포스팅날짜]/`에 안전하게 배치(필요시 자동 폴더 이동)합니다.
8. 최종적으로 Prettier를 호출하여 문단과 마크다운 가독성 서식을 매끄럽게 보정한 최종 결과 파일을 출력합니다.
