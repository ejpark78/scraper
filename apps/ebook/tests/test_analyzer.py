...
from src.split_chapter import BookProfile, ChapterDef
...
def test_analyze_directory_recursion(temp_raw_dir):
    ...
    with patch.object(analyzer, 'analyze') as mock_analyze:
        ...
        assert mock_analyze.call_count == 4  # 1 for dir + 3 for files
        called_paths = [call[0][0] for call in mock_analyze.call_args_list if call[0][0] != str(temp_raw_dir)]
        assert str(pdf1) in called_paths
        assert str(pdf2) in called_paths
        assert str(epub) in called_paths
...
def test_overwrite_skip_logic_true(temp_raw_dir):
    ...
    with patch('fitz.open') as mock_fitz_open:
        ...
        with patch.object(analyzer, '_save_books_config') as mock_save:
            ...
            assert result.title == "existing.pdf"
