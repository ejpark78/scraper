...
- **Monorepo Separation**: `apps/viewer/docker/compose.yml`을 `apps/viewer/compose.yml`로 통합/이동하고 내부 빌드 컨텍스트를 `.`로 정렬하여 결합도를 최소화함. 또한 `apps/viewer/Makefile`에 `down` 타겟을 구현하고 루트 `down-viewer`를 래핑 연동함.
