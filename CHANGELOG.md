...
### [1.12.0] - 2026-06-23
* **Git Flow 브랜치 모델 도입**: `AGENTS.md`에 Git Flow 브랜치 규격 및 에이전트 행동 지침을 명문화하고, 100줄 제한을 완화하여 중복/예제 위주로만 정리하도록 갱신. 자동 커밋 스크립트(`commit-changes.sh`)가 현재 체크아웃된 브랜치명(`feature/###-*`, `hotfix/###-*`)을 감지하여 커밋 메시지를 적절한 접두사로 빌드하도록 고도화.
