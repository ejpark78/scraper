...
export class PyTorchKRRefreshUrls {
    public async run(): Promise<void> {
        console.log('🔄 [PyTorch KR Refresh Urls] Starting precision recovery of uncollected targets...');
        ...
        try {
            ...
            // 2. 현재 Redis scrape_queue 에 존재하는 PyTorch KR URL 목록 추출
            ...
            for (const payloadStr of existingQueuePayloads) {
                try {
                    ...
                    if (payload.site === 'pytorch_kr' && payload.url) {
            ...
            console.log(`📥 Loaded ${existingQueueUrls.size} PyTorch KR URLs currently in Redis scrape_queue.`);

            // 3. 미수집 잔여 타겟 선별
            const uncollectedNews = await pytorchUrlsColl.find({
                id: { $nin: completedIds }
            }, {
            ...
            if (filteredJobs.length > 0) {
                ...
                for (let i = 0; i < payloads.length; i += chunkSize) {
                    ...
                    await redis.rpush(`scrape_queue:pytorch_kr:${priority}`, ...chunk);
                }
