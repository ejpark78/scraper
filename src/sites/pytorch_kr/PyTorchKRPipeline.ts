...
import { PyTorchKRMeta, PyTorchKRConverter } from './PyTorchKRConverter';

export class PyTorchKRPipeline extends BasePipeline<PyTorchKRMeta> {
    ...
    protected async saveResults(meta: PyTorchKRMeta, id: string, tempHtmlPath: string, _redisInstance?: any): Promise<{ targetDirName: string }> {
        ...
        try {
            ...
            console.log(`📡 [MongoDB Write] Successfully saved PyTorch KR ID ${id} to bronze.pytorch_kr and silver.pytorch_kr.`);
...
if (require.main === module) {
    ...
    const pipeline = new PyTorchKRPipeline();
    pipeline.run(urlsFile).catch(err => {
        console.error('Fatal PyTorch KR pipeline crash:', err);
        process.exit(1);
    });
}
