...
* **로컬 파일 최소화**: 상세 계획 및 태스크, 결과보고서는 Gitea 이슈 히스토리에 영구 보존되므로 로컬 `docs/artifacts/` 폴더에 마크다운 파일을 개별 생성하여 보존하지 않습니다.
* **의사 결정서(ADR) 보존**: 아키텍처나 중요 설계 결정사항과 같이 로컬에 영구 보관할 가치가 높은 문서화의 경우에만 선택적으로 `docs/artifacts/###-filename.adr.md` 형태로 로컬에 기록·커밋합니다.
* **Squash & Archive 정책 폐지**: 로컬 파일 생성이 최소화되므로 기존 `make agents-squash` 및 `###-###.archive.md` 압축/보관 절차는 더 이상 수행하지 않습니다.
