...
3. **Minimal File Scope**: No root grep/list_dir. Prefer `git ls-files` to map tracked files efficiently and minimize API/token consumption. Use step-by-step `list_dir` only for untracked folders (e.g. `data/`) or fallback. Use view_file only on target files.
