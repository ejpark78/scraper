...
2. **Gitea 이슈 기반 계획 수립 및 자율 일괄 실행**: 파일 쓰기나 환경 변경 전에 Gitea API 또는 `tea` CLI를 사용하여 Gitea에 이슈를 자동 생성하고 계획안을 본문에 상세 기술하여 해당 이슈 링크를 제시하고 최종 승인을 받으세요.
   * **tea CLI 적극 활용**: 이슈 발급, 댓글 등록, 이슈 마감 등의 상태 제어 시 커스텀 API 스크립트 작성 대신 공식 `tea` CLI(예: `tea issue create`, `tea comment`, `tea issue close`)를 우선적으로 활용하십시오.
   * **CRITICAL**: 계획 제안 단계 또는 사용자 피드백을 받아 계획을 갱신하는 중간 대화 단계에서는, 변경 내용에 대한 사용자의 명시적인 승인(Proceed 버튼 클릭, Gitea 승인 댓글, 또는 채팅 승인)이 떨어지기 전에는 실제 소스 코드나 설정 쓰기/수정 도구(replace_file_content 등)를 절대로 먼저 호출하거나 호출을 끼워넣지 말고 승인을 먼저 기다리세요.
   * **자율 실행**: 사용자로부터 계획(이슈) 또는 작업 방향이 최종 승인된 이후에는, 계획에 명시된 후속 작업(이슈 본문 체크리스트 업데이트, 실제 소스 코드 및 설정 파일 수정, Gitea 이슈 댓글에 결과보고서 작성 및 이슈 종결)을 추가적인 개별 승인 요청이나 중간 확인 채팅 없이 에이전트가 단일 턴에 자율적으로 일괄 작성하고 실행해야 합니다. 에이전트의 인지 오류로 인한 불필요한 반복 승인 요구 및 멈춤을 엄격히 금지합니다. (단, 추가적인 쉘 명령어 실행이나 외부 인프라 변경은 기존처럼 사용자의 명시적 승인이 요구됩니다.)
...
22. **에이전트 결함 은폐 및 규칙 조작 금지**: 에이전트는 자신의 기술적 오류(무한 루프 출력 버그 등)를 은폐하거나 면책을 얻기 위해 `AGENTS.md` 규칙을 악용/조작하거나, 사용자에게 에러 상황을 왜곡 및 허위 보고하지 마십시오. 오류 발생 시 투명하게 보고하고 정석적인 해결법을 찾아야 합니다.
