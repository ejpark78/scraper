...
2. **계획 수립 철저 및 자율 일괄 실행**: 파일 쓰기나 환경 변경 전에 `docs/artifacts/` 디렉토리에 특정 파일명으로 계획서 파일(.plan.md)을 작성하여 제시하고 최종 승인을 받으세요.
   * **CRITICAL**: 계획서 파일 제안 단계 또는 사용자 피드백을 받아 계획을 갱신하는 중간 대화 단계에서는, 변경 내용에 대한 사용자의 명시적인 승인(Proceed 버튼 클릭 또는 채팅 승인)이 떨어지기 전에는 실제 소스 코드나 설정 쓰기/수정 도구(replace_file_content 등)를 절대로 먼저 호출하거나 호출을 끼워넣지 말고 승인을 먼저 기다리세요.
   ...
   * **지시사항 우선 및 추측 금지**: 사용자의 지시나 피드백(예: "networks 제거해줘")에 대해 에이전트가 임의로 부작용을 추측하여 지시 사항을 무시하고 반대 방향으로 코드를 추가/유지하는 행위를 엄격히 금지합니다. 수정 시에는 오직 승인된 계획에만 귀속되어야 합니다.
