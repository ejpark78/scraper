...
* **Gitea 이슈 기반 단일 계획 수립 및 자율 일괄 실행**:
  ...
  * **Gitea MCP 및 API 활용**: 이슈 발행, 댓글 등록, 이슈 마감 등의 상태 제어 시 `gitea-mcp`를 최우선적으로 활용합니다. **CLI 명령어(tea, curl 등)를 사용해 수동으로 Gitea API를 제어하는 셸 호출은 엄격히 금지합니다.** 만약 MCP 기능이 불완전하거나 연결에 장애가 있다면 무음 복구를 진행하지 말고 즉시 사용자에게 보고하여 검증 절차를 거칩니다.
...
- **ENV 접근 금지**: `.env` 또는 `.env.*` 파일에 직접 접근하지 말고, `.env.example`을 참조하세요.
