...
- **작업 내용**:
  ...
  - **[AGENTS.md](../../AGENTS.md) 수정**:
    - 규칙 17번에 작업 종료 시 자동화를 극대화하기 위해 `commit-changes.sh --merge` 옵션을 활용하여 머지까지 완료하라는 지침을 명문화했습니다.
...
## 3. 검증 결과
- 수정한 스크립트 `scripts/agents/commit-changes.sh --merge`를 직접 적용하여, 현재 수정 사항을 커밋하고 `develop` 브랜치로 자동 복귀하여 머지를 검증할 예정입니다.
