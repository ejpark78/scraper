...
## 2. 🛠️ 구현 작업
- [x] [site.config.ts](file:///home/ejpark/workspace/scraper/apps/crawler/src/sites/linkedin/jobs/site.config.ts) 수정
...
## 3. 🧪 테스트 및 검증
- [x] `config.json`에서 `geo_enable`를 `false`로 설정한 후 테스트 실행 또는 검증 (코드 무결성 분석)
- [x] 변경 사항 유효성 검증 후 자동 Git 커밋 수행

## 4. 📝 결과 보고
- [x] 결과보고서(Walkthrough) 및 리뷰(Review) 문서 작성
