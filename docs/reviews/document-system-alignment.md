...
- **`ScraperWorker.ts` & `ConverterWorker.ts`**:
  - 기존 레거시 상대경로(`../../database/mongo`) 임포트를 모노레포 표준 패키지 별칭인 `@wiki/database`로 변경하여, 빌드 타임 및 런타임 상에서 모노레포 공통 `packages/database` 모듈을 정상 참조하도록 수정 완료 (완료)

---

## 4. 종합 의견 (Conclusion)
* 모노레포 개편 작업 이후 잔존해 있던 총 5개의 Makefile 스크립트 실행 경로 및 백그라운드 Worker들의 데이터베이스 모듈 참조 경로가 모노레포 표준 별칭(`@wiki/database`)으로 정상 수정되었음을 최종 확인했습니다.
