...
### 3. 파일 유지보수 오버헤드 감소
- **평가:** 파격적으로 줄어들었습니다.
- **상세:** 이전엔 사이트가 9개 늘어나면 9개의 `.mk` 파일이 프로젝트 루트에 존재해야 했으나, 이제 단 하나의 `package.json` 안에서 선언적으로 관리되므로 변경 이력과 형상 복잡도가 극대화로 단순해졌습니다. 또한 `apps/crawler/Makefile` 신규 구축을 통해 루트 Makefile의 구조가 매우 간결해지고 크롤러 전용 관리 명령어의 단독 빌드가 가능해졌습니다.
