...
- 파이썬 이북 프로젝트(`apps/ebook/pyproject.toml`)에 `[project.scripts]`를 등록하여 npm scripts의 단축 명령어 실행 방식처럼 `ebook-process`라는 CLI 엔트리포인트를 구성하였고, 상대 임포트가 깨지지 않도록 `apps/ebook/src/__init__.py` 패키지 마커를 생성하였습니다.
- 이와 연계하여 `apps/ebook/Makefile` 내의 실행 구문들도 `uv run ebook-process` 형태로 리팩토링하여 로컬 가상환경 및 도커 컴포즈 상에서 실행 일관성을 확보했습니다.
...
- `apps/ebook` 디렉토리 내에서 `uv run ebook-process --summary` 명령어와 같이 npm scripts처럼 CLI를 통해 래핑 동작이 잘 수행되는 구조가 마련되었습니다.
- `make ebook-summary` (또는 `apps/ebook` 내 `make summary`) 호출 시 도커 컨테이너 내부에서도 `uv run ebook-process` 규칙을 타서 에러 없이 정상적으로 서머리를 출력합니다.
