...
- 루트 `Makefile`에는 포워딩 게이트웨이 포트 룰만 남겨두어, 사용자가 루트 디렉토리에서 기존 명령어(`make gpt-list`, `make test-recursive SITE=yozm`)를 입력하더라도 문제 없이 `apps/crawler/Makefile`로 패스되어 실행되도록 보장하였습니다.
